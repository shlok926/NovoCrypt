...
function runAdvisorTests() {
  ...
  // Test 1: Security Explanations
  console.log('\nTest 1: Vulnerability explanation templates reasoning');
  const sqliExplanation = orchestrator.generateSecurityExplanations('sql_injection', 'src/db.ts', ['input read', 'concatenation', 'sink execution']);
  sqliExplanation.then(({ summary, metrics }) => {
    ...
    console.log('  ✔ SQL Injection explanation narrative compiled.');

    // Test 2: Root Cause Analysis
    console.log('\nTest 2: Root cause analysis pattern detection');
    const rootCause = RootCauseAnalyzer.analyze('sql_injection', 'src/db.ts');
    assert.strictEqual(rootCause.vulnerabilityId, 'sql_injection');
    assert(rootCause.pattern.includes('concatenation'));
    ...
    const recs = orchestrator.generateRecommendations('sql_injection');
    ...
    // Test 5: Fix Comparison
    console.log('\nTest 5: Remediate strategy comparative analysis');
    const comparisons = orchestrator.compareRemediationStrategies(['parameterize', 'orm']);
    ...
    // Test 9: Developer Guidance
    console.log('\nTest 9: Level-specific guidance details');
    const insights = orchestrator.generateDeveloperGuidance('sql_injection');
    ...
    for (let i = 0; i < 10000; i++) {
      orchestrator.generateRecommendations('sql_injection');
    }
