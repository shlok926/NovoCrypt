...
export class ReportEngine {
  ...
  async generateReportBuffer(context: ReportContext): Promise<Buffer> {
    ...
    return new Promise(async (resolve, reject) => {
          ...
          try {
            ...
            if (section.isAuthorized) {
              ...
              if (!authorized) {
                ...
                metrics.sectionsSkipped++;
                continue;
              }
            }

            metrics.sectionsAttempted++;
            ...
            metrics.sectionsSucceeded++;
          } catch (sectionError) {
            metrics.sectionsFailed++;
      ...
      const pdfSizeMB = (buffer.length / (1024 * 1024)).toFixed(2);
      
      console.log(`[ReportEngine Metrics] Generated report for user ${context.userId}:
- Duration: ${(durationMs / 1000).toFixed(2)}s
- Sections Succeeded: ${metrics.sectionsSucceeded}
- Sections Failed: ${metrics.sectionsFailed}
- Sections Skipped: ${metrics.sectionsSkipped}
- PDF Size: ${pdfSizeMB} MB`);
  ...
  private renderFooter(doc: PDFKit.PDFDocument) {
    ...
    for (let i = 0; i < pages; i++) {
      ...
      doc.fontSize(8).fillColor('#94a3b8').text(
        'Generated by NovoCrypt Executive Reporting Engine v1.0.0 | Report Version: 1.0',
        50,
        doc.page.height - 40,
